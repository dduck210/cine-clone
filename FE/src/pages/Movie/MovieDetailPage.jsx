...
import { Star, Clock, Calendar, User, MapPin, ChevronDown, ChevronUp, Info, Ticket, Play } from "lucide-react";
...
const toVNDateStr = (d = new Date()) => new Date(+d + 7 * 60 * 60 * 1000).toISOString().split("T")[0];
...
const MovieDetailPage = () => {
    ...
    (async () => {
      try {
        ...
        showtimeRes.data.forEach((st) => {
          ...
          if (!grouped[cid]) grouped[cid] = { id: cid, name: st.cinema.name, address: st.cinema.address, showtimes: [], isOpen: false };
  ...
  const handleSelectTime = (showtime, cinema) =>
    setSelectedShowtime({ showtimeId: showtime._id, time: showtime.startTime, date: showtime.date, cinemaId: cinema.id, cinemaName: cinema.name, address: cinema.address, price: showtime.price, roomName: showtime.room?.name || "" });
    ...
    <div className="min-h-screen bg-white font-sans text-gray-900">
      ...
      <div className="relative pt-16 md:pt-20 overflow-hidden bg-slate-900">
        ...
        <div className="absolute inset-0">
          <img src={movie.poster} alt="" className="w-full h-full object-cover scale-110 blur-3xl opacity-25" />
              ...
              {getYoutubeId(movie.trailer) && (
                ...
                >
                  <span className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center shadow-2xl">
                    <Play className="text-[#dc2626] ml-1" size={28} fill="currentColor" />
                  </span>
                </button>
              )}
            </div>
          ...
          <div className="flex-1 text-white pb-6">
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              {movie.status === "now_showing" && (
                <span className="bg-[#dc2626] text-white text-[11px] font-black px-2.5 py-1 rounded-md uppercase tracking-widest">Đang chiếu</span>
              )}
              {movie.status === "coming_soon" && (
                <span className="bg-amber-500 text-white text-[11px] font-black px-2.5 py-1 rounded-md uppercase tracking-widest">Sắp chiếu</span>
              )}
              {movie.ageRestriction && (
                <span className="border border-white/30 text-white/80 text-[11px] font-bold px-2.5 py-1 rounded-md">{movie.ageRestriction}</span>
              )}
            </div>

            <h1 className="text-3xl md:text-[2.6rem] font-black leading-tight mb-4">{movie.title}</h1>
              ...
              {movie.rating > 0 && (
                <div className="flex items-center gap-1.5">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <Star key={i} size={14} fill={i <= Math.round(movie.rating / 2) ? "currentColor" : "none"} className="text-yellow-400" />
                    ))}
                  </div>
                  <span className="text-yellow-300 font-bold">{movie.rating}</span>
                  <span className="text-white/40 text-xs">/10</span>
                </div>
              )}
              <span className="text-white/25">|</span>
              <span className="flex items-center gap-1 text-white/70"><Clock size={13} />{durationLabel}</span>
              {movie.releaseDate && (
                <><span className="text-white/25">|</span>
                  <span className="flex items-center gap-1 text-white/70"><Calendar size={13} />{new Date(movie.releaseDate).getFullYear()}</span>
                </>
              )}
              {genreNames.map((g) => (
                <span key={g} className="bg-white/10 hover:bg-white/20 text-white/80 text-xs px-2.5 py-1 rounded-full transition-colors">{g}</span>
              ))}
            </div>

            <p className="text-white/70 leading-relaxed mb-6 max-w-2xl text-[15px] line-clamp-3">{movie.description || "Chưa có mô tả."}</p>

            {/* Director / Cast / Release */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4 mb-8 max-w-xl text-sm">
              {movie.director && (
                <div>
                  <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold mb-1 flex items-center gap-1"><Info size={10} />Đạo diễn</p>
                  <p className="text-white font-semibold">{movie.director}</p>
                </div>
              )}
              {movie.cast && (
                <div>
                  <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold mb-1 flex items-center gap-1"><User size={10} />Diễn viên</p>
                  <p className="text-white font-semibold line-clamp-2">{movie.cast}</p>
                </div>
              )}
              {movie.releaseDate && (
                <div>
                  <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold mb-1 flex items-center gap-1"><Calendar size={10} />Khởi chiếu</p>
                  <p className="text-white font-semibold">{new Date(movie.releaseDate).toLocaleDateString("vi-VN")}</p>
                </div>
              )}
            </div>
            ...
            {selectedShowtime ? (
              ...
              <button
                onClick={() => {
                  ...
                  if (el) {
                    el.classList.add("ring-2", "ring-[#dc2626]", "ring-offset-4", "rounded-xl");
      ...
      <main className="container mx-auto px-6 pb-12 max-w-6xl">
        ...
        {movie.description && (
          <section className="py-10 border-b border-gray-100">
            <h2 className="text-xl font-bold text-gray-900 mb-4 border-l-4 border-[#dc2626] pl-3">Nội dung phim</h2>
            <p className="text-gray-600 leading-relaxed max-w-3xl text-[15px] whitespace-pre-line">{movie.description}</p>
          </section>
        )}
        ...
        {movie.cast && (
          <section className="py-10 border-b border-gray-100">
            <h2 className="text-xl font-bold text-gray-900 mb-6 border-l-4 border-[#dc2626] pl-3">Diễn viên tham gia</h2>
            <div className="flex flex-wrap gap-3">
              {movie.cast.split(",").map((name) => name.trim()).filter(Boolean).map((actor) => {
                const initials = actor.split(" ").slice(-2).map((w) => w[0]?.toUpperCase()).join("");
                const colors = ["bg-red-100 text-red-700", "bg-blue-100 text-blue-700", "bg-violet-100 text-violet-700", "bg-amber-100 text-amber-700", "bg-emerald-100 text-emerald-700", "bg-pink-100 text-pink-700"];
                const color = colors[actor.charCodeAt(0) % colors.length];
                return (
                  <div key={actor} className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 hover:border-red-300 hover:bg-red-50 transition-colors group">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm shrink-0 ${color}`}>
                      {initials}
                    </div>
                    <span className="text-gray-800 font-semibold text-sm group-hover:text-[#dc2626] transition-colors">{actor}</span>
                  </div>
                );
              })}
            </div>
        ...
        {/* ── SHOWTIMES ── */}
        <section ref={showtimeSectionRef} className="py-10 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-4 border-l-4 border-[#dc2626] pl-3">Lịch Chiếu Phim</h2>

          <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
            {Array.from({ length: 7 }).map((_, i) => {
              const d = new Date(+new Date() + i * 24 * 60 * 60 * 1000);
              const val = toVNDateStr(d);
              const label = i === 0 ? "Hôm nay" : i === 1 ? "Ngày mai" : `${WEEKDAY[vnDay(d)]} ${vnDate(d)}/${vnMonth(d) + 1}`;
              return (
                <button key={val} onClick={() => setSelectedDate(val)}
                  className={`flex-shrink-0 px-4 py-2 rounded-xl font-bold text-sm transition-all border ${selectedDate === val ? "bg-[#dc2626] text-white border-[#dc2626] shadow-md" : "bg-white text-gray-600 border-gray-200 hover:border-red-400 hover:text-red-600"}`}>
                  {label}
                </button>
              );
            })}
          ...
          {cinemaList.length === 0 ? (
            ...
            <div className="space-y-4">
              {cinemaList.map((cinema) => {
                ...
                return (
                  <div key={cinema.id} className={`border rounded-xl overflow-hidden transition-all bg-white ${cinema.isOpen ? "border-red-300 shadow-md" : "border-gray-200"}`}>
                    <div className={`flex justify-between items-center p-5 cursor-pointer ${cinema.isOpen ? "bg-red-50" : "bg-white hover:bg-gray-50"}`} onClick={() => toggleCinema(cinema.id)}>
                      <div className="flex items-center gap-4">
                        <div className={`p-2 rounded-full ${cinema.isOpen ? "bg-red-100 text-[#dc2626]" : "bg-gray-100 text-gray-400"}`}><MapPin size={20} /></div>
                        <div>
                          <h3 className={`font-bold text-lg ${cinema.isOpen ? "text-[#dc2626]" : "text-gray-800"}`}>{cinema.name}</h3>
                          <p className="text-gray-500 text-xs mt-0.5">{cinema.address}</p>
                        </div>
                      </div>
                      {cinema.isOpen ? <ChevronUp size={20} className="text-gray-400" /> : <ChevronDown size={20} className="text-gray-400" />}
                    </div>
                    {cinema.isOpen && (
                      <div className="p-5 pt-0 bg-red-50/30 border-t border-red-100">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {filtered.map((showtime) => {
                            ...
                            return (
                              <button key={showtime._id} onClick={() => handleSelectTime(showtime, cinema)}
                                ...
                                {showtime.startTime}
                                {showtime.availableSeats !== undefined && <span className="ml-2 text-xs opacity-70">({showtime.availableSeats} ghế)</span>}
                              </button>
                            );
                          })}
                        </div>
        ...
        {/* ── TRAILER ── */}
        {getYoutubeId(movie.trailer) && (
          <section id="trailer-section" className="py-10 border-b border-gray-100">

    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          <p className="text-2xl font-bold text-gray-400">Không tìm thấy bài viết</p>
          ...
          <article className="prose prose-lg max-w-none">
            {article.content.map((block, i) => {
              if (block.type === "heading") {
                return (
                  <h2 key={i} className="text-xl md:text-2xl font-extrabold text-gray-900 mt-8 mb-3">
                    {block.text}
                  </h2>
                );
              }
              if (block.type === "rating") {
                return (
                  <div key={i} className="mt-8 flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-6 py-4">
                    <Tag size={20} className="text-red-600 shrink-0" />
                    <span className="font-bold text-gray-700">Điểm đánh giá:</span>
                    <span className="text-2xl font-extrabold text-red-600">{block.score}</span>
                  </div>
                );
              }
              return (
                ...
                {related.map((item) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <h4 className="font-bold text-gray-800 mt-1 text-sm line-clamp-2 group-hover:text-red-600 transition-colors">
                        {item.title}
                      </h4>

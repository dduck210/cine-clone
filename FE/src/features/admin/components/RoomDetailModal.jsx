...
import { MatrixView } from "@/features/admin/components/MatrixEditor";
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 dark:border-gray-700 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">Chi tiết phòng chiếu</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Kích thước", value: `${matrix.length} hàng × ${matrix[0]?.length || 0} cột` },
              { label: "Tổng ghế", value: `${room.totalSeats} ghế` },
              { label: "Trạng thái", value: room.status === "active" ? "Hoạt động" : "Bảo trì", isStatus: true },
            ].map((item) => (
              <div key={item.label} className="bg-slate-50 dark:bg-gray-700 rounded-xl p-4 border border-slate-100 dark:border-gray-600">
                ...
                {item.isRoomType ? (
                  ...
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${room.status === "active" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-200"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${room.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-amber-400"}`} />
                    {item.value}
                  </span>
                ) : (
          ...
          {typeCount && (
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 dark:bg-gray-700 rounded-xl p-4 border border-slate-200 dark:border-gray-600 text-center">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-gray-500 mb-1">Ghế Thường</p>
                <p className="text-2xl font-black text-slate-700 dark:text-gray-200">{typeCount.normal}</p>
              </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <p className="text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider">Sơ đồ ghế</p>
              {!hasMatrix && (

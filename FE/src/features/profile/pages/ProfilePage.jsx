...
import { User, Mail, Camera, Star, Lock, History, ChevronRight, Save, Eye, EyeOff, Ticket, MapPin, Calendar, Phone, Heart } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 font-sans text-gray-900 dark:text-white">
      ...
      <div className="container mx-auto px-4 pt-24 md:pt-28 pb-12 max-w-6xl">
        <div className="text-sm text-gray-500 dark:text-gray-400 mb-6">Trang chủ / <span className="text-gray-900 dark:text-white font-medium">Tài khoản của tôi</span></div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center text-center relative overflow-hidden">
              ...
              <div className="relative z-10 mb-3 group cursor-pointer mt-4">
                <div className="relative">
                  ...
                  <div className="absolute bottom-0 right-0 bg-gray-900 dark:bg-gray-700 text-white p-1.5 rounded-full border-2 border-white dark:border-gray-600 hover:bg-red-600 transition-colors"><Camera size={14} /></div>
                </div>
              </div>
              <h2 className="relative z-10 text-xl font-bold text-gray-800 dark:text-white">{user.name}</h2>
              <div className="relative z-10 flex items-center gap-2 mt-2">
                <span className="bg-yellow-100 text-yellow-700 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 border border-yellow-200"><Star size={12} fill="currentColor" /> {user.rank} Member</span>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              {[
                ...
                <button key={id} onClick={() => setActiveTab(id)}
                  className={`w-full flex items-center justify-between p-4 text-sm font-medium transition-all duration-200 border-b border-gray-50 dark:border-gray-700 last:border-0 ${activeTab === id ? "bg-red-50 dark:bg-red-900/20 text-red-600 border-l-4 border-l-red-600 pl-3" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 border-l-4 border-l-transparent pl-4"}`}>
...
const PersonalInfoTab = ({ storedUser }) => {
  ...
  const setField = (field) => (e) => { setForm((p) => ({ ...p, [field]: e.target.value })); if (errors[field]) setErrors((p) => ({ ...p, [field]: "" })); };
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input type="tel" value={form.phone} onChange={setField("phone")} placeholder="0912 345 678"
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input type="email" value={storedUser?.email || ""} disabled className="w-full pl-10 pr-4 py-2.5 bg-gray-100 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg text-gray-500 dark:text-gray-400 cursor-not-allowed text-sm" />
          </div>
...
const HistoryTab = ({ navigate }) => {
  ...
  const handleViewDetail = (booking) => {
    ...
    const sharedState = {
      movieTitle: movie.title || "Phim", cinemaName: cinema.name || "5Cine", roomName: showtime.room?.name || "",
      showTime: showtime.startTime || "", showDate: showtime.date ? new Date(showtime.date).toLocaleDateString("vi-VN") : "",
      showAddress: cinema.address || "", selectedSeats: booking.seatNumbers || [], combos: booking.extraItems || [],
      finalTotalPrice: booking.totalPrice, poster: movie.poster || "", duration: movie.duration || 0,
    };
    if (booking.status === "pending") { navigate("/payment", { state: { ...sharedState, existingBookingId: booking._id, existingBookingCode: booking.bookingCode } }); return; }
    navigate("/payment-success", { state: { ...sharedState, orderId: booking.bookingCode, bookingId: booking._id, isHistoryMode: true, bookingStatus: booking.status, ticketStatus: booking.ticketStatus || "not_printed", paymentMethod: booking.paymentId?.method || "" } });
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {paidBookings.length > 0 && <span className="text-xs font-semibold text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">{paidBookings.length} giao dịch</span>}
      </div>
      {loading ? (
        <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-20 bg-gray-100 dark:bg-gray-700 rounded-2xl animate-pulse" />)}</div>
      ) : paidBookings.length === 0 ? (
        <div className="text-center py-16"><Ticket size={40} className="text-gray-300 mx-auto mb-3" /><p className="text-gray-500 font-medium">Bạn chưa có giao dịch nào.</p></div>
      ) : (
        <div className="space-y-3">
          {paidBookings.map((b) => {
            const showtime = b.showtime || {}; const movie = showtime.movie || {}; const cinema = showtime.cinema || {};
            ...
            return (
              <div key={b._id} onClick={() => handleViewDetail(b)} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 hover:bg-red-50/60 dark:hover:bg-red-900/20 border border-gray-100 dark:border-gray-600 hover:border-red-200 dark:hover:border-red-700 rounded-2xl cursor-pointer transition-all group">
                <div className="w-10 h-14 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-600 flex-shrink-0 shadow-sm">
                  {movie.poster ? <img src={movie.poster} alt={movie.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center"><Ticket size={16} className="text-gray-400" /></div>}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1">
                    ...
                    {dateStr && <span className="text-xs text-gray-400 flex items-center gap-1"><Calendar size={11} /> {dateStr}{showtime.startTime ? ` · ${showtime.startTime}` : ""}</span>}
                  </div>
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  {b.paymentId?.method && methodMap[b.paymentId.method] && (
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${methodMap[b.paymentId.method].color}`}>{methodMap[b.paymentId.method].label}</span>
                  )}
                </div>
              </div>
            );
...
const ChangePasswordTab = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.currentPassword || !form.newPassword || !form.confirmPassword) { toast.error("Vui lòng điền đầy đủ thông tin"); return; }
    if (form.newPassword !== form.confirmPassword) { toast.error("Mật khẩu xác nhận không khớp"); return; }
    if (form.newPassword.length < 8) { toast.error("Mật khẩu mới phải có ít nhất 8 ký tự"); return; }
    setSaving(true);
    try {
      await axiosInstance.put("/auth/change-password", { currentPassword: form.currentPassword, newPassword: form.newPassword });
  ...
  const PasswordField = ({ label, field, showKey }) => (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input type={show[showKey] ? "text" : "password"} value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} placeholder={`Nhập ${label.toLowerCase()}`}
        ...
        <button type="button" onClick={() => setShow({ ...show, [showKey]: !show[showKey] })} className="absolute right-3 top-3 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="pt-4">
          <button type="submit" disabled={saving}
            ...
            {saving ? <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg> : <Lock size={18} />}
...
const WishlistTab = () => {
  ...
  useEffect(() => {
    axiosInstance.get("/auth/wishlist").then((res) => setMovies(res.data)).catch(() => setMovies([])).finally(() => setLoading(false));
  }, []);

  const handleRemove = async (movieId) => { await toggle(movieId); setMovies((prev) => prev.filter((m) => m._id !== movieId)); };

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white border-l-4 border-red-600 pl-3">Phim yêu thích</h2>
        {movies.length > 0 && <span className="text-xs font-semibold text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">{movies.length} phim</span>}
      </div>
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{[1,2,3,4].map(i => <div key={i} className="animate-pulse"><div className="aspect-[2/3] rounded-2xl bg-gray-200 dark:bg-gray-700" /></div>)}</div>
      ) : movies.length === 0 ? (
        <div className="text-center py-16"><Heart size={40} className="text-gray-200 mx-auto mb-3" /><p className="text-gray-500 font-medium">Chưa có phim yêu thích.</p><Link to="/movies" className="mt-3 inline-block text-sm text-red-600 font-bold hover:underline">Khám phá phim ngay →</Link></div>
      ) : (
            ...
            <div key={movie._id} className="group relative">
              <Link to={`/movie/${movie._id}`} className="block">
                <div className="relative aspect-[2/3] rounded-2xl overflow-hidden bg-gray-200 dark:bg-gray-600 shadow-sm group-hover:shadow-lg transition-all">
                  <img src={movie.poster} alt={movie.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onError={(e) => { e.target.src = "https://via.placeholder.com/300x450?text=No+Image"; }} />
                </div>
                <p className="mt-2 text-sm font-bold text-gray-800 dark:text-white line-clamp-2 group-hover:text-red-600 transition-colors">{movie.title}</p>
              </Link>
              <button onClick={() => handleRemove(movie._id)} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-red-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity active:scale-90" title="Bỏ yêu thích">

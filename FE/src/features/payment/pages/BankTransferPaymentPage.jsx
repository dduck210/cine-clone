...
const BankTransferPaymentPage = () => {
  ...
  const [qrPhase, setQrPhase] = useState(true);
  ...
  const CopyRow = ({ label, value }) => (
    <div className="flex items-center justify-between bg-slate-50 dark:bg-gray-700 rounded-xl px-4 py-3 border border-slate-100 dark:border-gray-600">
      ...
      <button onClick={() => copyText(value, label)} className="text-[#004C97] hover:text-[#003a75] transition-colors p-1 rounded-lg hover:bg-blue-50 dark:hover:bg-gray-600">
        <Copy size={16} />
      </button>
    </div>
  );
      ...
      <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-24 pb-20">
            ...
            <div className="bg-white dark:bg-gray-800 w-full rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 dark:border-gray-700 flex flex-col">
              <div className="bg-gradient-to-r from-[#004C97] to-[#0066CC] p-7 text-center text-white relative">
                <div className="flex items-center justify-center gap-3 mb-2">
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md"><Ticket size={22} className="text-white" /></div>
              ...
              <div className="flex flex-col items-center pb-4 border-b border-dashed border-slate-200">
                ...
                <div className="bg-slate-50 dark:bg-gray-700 p-3 rounded-[2rem] border border-slate-100 dark:border-gray-600 shadow-inner flex items-center justify-center w-[186px] h-[186px]">
                  {qrError ? (
                    ...
                    <img src={qrUrl} alt="MB Bank QR" className="w-[160px] h-[160px]" onError={() => setQrError(true)} />
                  )}
                </div>
          ...
          <div className="hidden lg:flex lg:w-[52%]">
            <div className="bg-white w-full rounded-[2rem] shadow-2xl border border-slate-100 p-10 flex flex-col items-center justify-center relative overflow-hidden">
              <div className="flex flex-col items-center animate-in fade-in zoom-in duration-700 w-full">
                <div className="bg-[#004C97] text-white px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-8 shadow-xl shadow-blue-200">Quét mã để thanh toán</div>
                <div className="relative group">
                  ...
                  <div className="relative bg-white p-5 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100 flex items-center justify-center w-[270px] h-[270px]">
                    {qrError ? (
                      ...
                      <img src={qrUrl} alt="MB Bank QR" className="w-[240px] h-[240px]" onError={() => setQrError(true)} />
                    )}
                  </div>
                </div>
                ...
                <div className="mt-6 flex flex-col items-center gap-3">
                  ...
                  <p className="text-slate-400 text-[10px] max-w-[320px] text-center pt-2 border-t border-slate-100">Hệ thống tự động phát hiện thanh toán. Vui lòng không tắt trình duyệt cho đến khi nhận được vé.</p>
                </div>
              </div>
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-6 px-8 py-3 bg-white rounded-full shadow-sm border border-slate-100">
            <div className="flex items-center gap-2 text-slate-400 text-[10px] font-bold uppercase tracking-widest"><ShieldCheck size={14} className="text-emerald-500" /> Thanh toán an toàn</div>
            <div className="w-px h-4 bg-slate-200" />
            <div className="flex items-center gap-2 text-slate-400 text-[10px] font-bold uppercase tracking-widest"><Clock size={14} className="text-blue-500" /> Hỗ trợ 24/7</div>
          </div>
        </div>
      </main>
      <Footer />

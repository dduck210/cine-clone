...
const CinemaDetailPage = () => {
  ...
  const handleBooking = (showtime, movie) => {
    navigate(`/booking/${movie._id}`, {
      state: {
        selectedShowtime: {
          showtimeId: showtime._id, time: showtime.startTime, date: showtime.date,
          cinemaId: id, cinemaName: cinema?.name || "", address: cinema?.address || "",
          price: showtime.price, roomName: showtime.room?.name || "", duration: movie.duration || 0,
        },
        selectedDate, movieTitle: movie.title, poster: movie.poster,
      },
    });
  };
  ...
  const handleOpenMap = () => {
    ...
    window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(cinema.address)}`, "_blank");
  };

  const filteredGroups = movieGroups.map((g) => ({
    ...g,
    showtimes: g.showtimes.filter((st) => !selectedDate || toVNDateStr(new Date(st.date)) === selectedDate),
  ...
  for (let i = 0; i < 7; i++) {
    ...
    dateOptions.push({ value: toVNDateStr(d), label: WEEKDAY[vnDay(d)], fullDate: `${vnDate(d)}/${vnMonth(d)+1}` });
  }
      ...
      {loading ? (
        ...
        <>
          <div className="relative h-[260px] sm:h-[340px] md:h-[420px] overflow-hidden mt-16 md:mt-20">
            <img src={cinema.image || FALLBACK_IMAGE} alt={cinema.name} onError={(e) => { e.target.src = FALLBACK_IMAGE; }} className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
            <Link to="/cinemas" className="absolute top-4 left-4 sm:left-8 flex items-center gap-1.5 text-white/90 hover:text-white text-sm font-bold bg-black/30 backdrop-blur-sm px-3 py-2 rounded-full transition-all hover:bg-black/50">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-12">
            <div className="flex border-b border-gray-200 dark:border-gray-700 mb-5 sticky top-16 md:top-20 bg-gray-50 dark:bg-gray-950 z-10">
              {[{ key: "showtimes", icon: <Calendar size={15} />, label: "Lịch Chiếu" }, { key: "info", icon: <Info size={15} />, label: "Thông Tin Rạp" }].map(({ key, icon, label }) => (
                <button key={key} onClick={() => setActiveTab(key)}
                  className={`flex items-center gap-2 px-5 py-4 text-sm font-bold border-b-2 -mb-px transition-colors ${activeTab === key ? "border-red-600 text-red-600" : "border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200"}`}>
                  {icon}{label}
                </button>
              ))}
            </div>
            ...
            {activeTab === "showtimes" ? (
                ...
                <div className="flex gap-2 overflow-x-auto pb-3 no-scrollbar mb-5">
                  {dateOptions.map((d) => (
                    <button key={d.value} onClick={() => setSelectedDate(d.value)}
                      className={`flex-shrink-0 px-3 py-2.5 rounded-xl border font-bold text-xs transition-all flex flex-col items-center gap-0.5 min-w-[68px] ${selectedDate === d.value ? "bg-red-600 border-red-600 text-white shadow-lg shadow-red-100" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-red-400 hover:text-red-600"}`}>
                      ...
                      <div key={movie._id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex">
                          <div className="w-[90px] sm:w-[110px] shrink-0 bg-gray-900">
                            <img src={movie.poster} alt={movie.title} className="w-full h-full object-cover" style={{ minHeight: '130px' }} onError={(e) => { e.target.src = "https://via.placeholder.com/200x300?text=No+Image"; }} />
                          </div>
                          <div className="flex-1 p-4 sm:p-5">
                            <div className="flex flex-wrap items-center gap-2 mb-1.5">
                              {movie.ageRestriction && <span className="bg-orange-500 text-white text-[10px] px-1.5 py-0.5 rounded font-black">{movie.ageRestriction}</span>}
                            ...
                            <div className="flex flex-wrap gap-2">
                              {showtimes.map((st) => (
                                <button key={st._id} onClick={() => handleBooking(st, movie)}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-3 sm:gap-4">
                  {[{ value: "6+", label: "Phòng chiếu", icon: <Film size={18} /> }, { value: "1.200+", label: "Chỗ ngồi", icon: <Ticket size={18} /> }, { value: "Premium", label: "Hạng rạp", icon: <Star size={18} /> }].map((s, i) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2 space-y-5">
                    <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 border-l-4 border-l-red-600 shadow-sm px-6 py-5">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 leading-relaxed text-sm">Chào mừng bạn đến với <span className="font-bold text-gray-800 dark:text-gray-200">{cinema.name}</span> — một trong những rạp chiếu phim hiện đại bậc nhất trong hệ thống 5Cine.</p>
                    </section>
                    <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 sm:p-6">
                      <h3 className="font-black text-gray-900 dark:text-white mb-4 flex items-center gap-2"><span className="w-1 h-5 bg-red-600 rounded-full" />Tiện Ích Tại Rạp</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                        {amenities.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
                            <div className="w-9 h-9 rounded-xl bg-red-50 dark:bg-red-900/20 flex items-center justify-center text-red-500 shrink-0">{item.icon}</div>
                            <div><p className="font-bold text-gray-800 dark:text-gray-200 text-sm leading-tight">{item.label}</p><p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{item.desc}</p></div>
                          </div>
                        ))}
                      </div>
                    </section>
                  ...
                  <div className="space-y-5">
                    <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                      <div className="h-48 relative cursor-pointer group" onClick={handleOpenMap}>
                        <iframe title="map" src={`https://maps.google.com/maps?q=${encodeURIComponent(cinema.address)}&t=&z=16&ie=UTF8&iwloc=&output=embed`} className="w-full h-full border-0 pointer-events-none" loading="lazy" />
                        <div className="absolute inset-0 bg-transparent group-hover:bg-black/10 transition-colors flex items-end justify-center pb-3">
                          <span className="bg-white/90 text-xs font-semibold px-3 py-1.5 rounded-full text-gray-700 shadow opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5"><Navigation size={11} /> Mở Google Maps</span>
                        </div>
                      </div>
                      <div className="p-4 space-y-3">
                        <div className="flex items-start gap-2"><MapPin size={14} className="text-red-500 shrink-0 mt-0.5" /><p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">{cinema.address}</p></div>
                        <button onClick={handleOpenMap} className="w-full py-2.5 bg-gray-900 dark:bg-gray-700 text-white rounded-xl font-bold text-xs hover:bg-red-600 dark:hover:bg-red-600 transition-all flex items-center justify-center gap-1.5"><Navigation size={13} /> Chỉ Đường</button>
                      </div>
                    </section>
                    <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                      <div className="bg-gray-900 px-5 py-4">
                        <p className="text-gray-400 text-xs font-semibold mb-1">Hotline đặt vé</p>
                        <div className="flex items-center gap-2 text-white font-black text-xl"><Phone size={18} className="text-red-400" />{cinema.phone || "1900 6606"}</div>
                      </div>
                      <div className="p-4 space-y-2 border-b border-gray-100 dark:border-gray-700">
                        ...
                        {[{ day: "Thứ 2 — Thứ 6", hours: "09:00 – 23:00" }, { day: "Thứ 7 — Chủ nhật", hours: "08:00 – 24:00" }].map((h, i) => (
                      ...
                      <div className="p-4">
                        <button onClick={() => setActiveTab("showtimes")} className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-xs transition-colors flex items-center justify-center gap-1.5"><Ticket size={13} /> Xem Lịch Chiếu</button>
                      </div>
                    </section>
                  </div>
                </div>

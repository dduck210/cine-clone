...
export const RoomsManager = ({ cinemas }) => {
  ...
  const handleCinemaStatusChange = async (status) => {
    ...
    const confirmMsg = status === "incident"
      ? `Chuyển rạp sang Bảo trì sẽ huỷ toàn bộ suất chiếu sắp tới và hoàn tiền các đơn đã thanh toán.\n\nXác nhận?`
      : `Mở lại rạp sẽ chuyển tất cả phòng về trạng thái hoạt động.\n\nXác nhận?`;
    if (!window.confirm(confirmMsg)) return;

    try {
      const res = await axiosInstance.patch(`/admin/cinemas/${selectedCinema}/status`, { status });
                  ...
                  return (
                    <tr key={room._id} className="hover:bg-slate-50/80 transition-colors cursor-pointer" onClick={() => setDetailRoom(room)}>
                      ...
                      <td className="p-4 text-center">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold border ${room.status === "active" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-200"}`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${room.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-amber-400"}`}></span>

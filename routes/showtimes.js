...
router.post('/', protect, admin, async (req, res) => {
    ...
    try {
        ...
        if (!room) return res.status(404).json({ message: 'Phòng chiếu không tồn tại' });
...
router.put('/:id', protect, admin, async (req, res) => {
    try {
        ...
        if (showtime) {
            ...
            res.status(404).json({ message: 'Suất chiếu không tồn tại' });
        }
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
router.put('/:id/cancel', protect, admin, async (req, res) => {
    try {
        ...
        if (showtime) {
            res.json({ message: 'Hủy suất chiếu thành công', showtime });
        } else {
            res.status(404).json({ message: 'Suất chiếu không tồn tại' });
        }
    } catch (error) {
